#!/usr/bin/env python3
"""
Measure real-world placement poses for conservative_002.

Workflow for each digital-twin step:
...
Run on JetMax:
    rosrun jetmax_demos tune_conservative002_place_poses.py --start-step 1 --redo-existing
...
Keys while tuning a placement:
    ...
    [/] : move step -/+   minimum 0.2mm; / also increases step
...
ROS_NODE_NAME = "tune_conservative002_place_poses"
OUT_PATH = os.path.join("calibration", "conservative_002_place_poses.json")
...
CONSERVATIVE_002 = [
    {"step": 1, "brick": "C", "side": "L", "wx": 8.0, "wz": 1.5642247142857144},
    {"step": 2, "brick": "A", "side": "L", "wx": 15.0, "wz": 1.5642247142857142},
    {"step": 3, "brick": "F", "side": "L", "wx": 8.0, "wz": 2.801403862549198},
    {"step": 4, "brick": "B'", "side": "L", "wx": 17.0, "wz": 3.4199934366809397},
    {"step": 5, "brick": "B", "side": "L", "wx": 12.5, "wz": 4.657172584944423},
    {"step": 6, "brick": "E'", "side": "L", "wx": 18.0, "wz": 4.657172584944423},
    {"step": 7, "brick": "F", "side": "L", "wx": 12.5, "wz": 5.894351733207906},
    {"step": 8, "brick": "C'", "side": "L", "wx": 20.0, "wz": 6.5129413073396485},
    {"step": 9, "brick": "B'", "side": "L", "wx": 12.5, "wz": 7.750120455603132},
    {"step": 10, "brick": "B", "side": "L", "wx": 11.0, "wz": 8.987299603866616},
    {"step": 11, "brick": "F'", "side": "L", "wx": 18.5, "wz": 8.368710029734874},
    {"step": 12, "brick": "D", "side": "L", "wx": 14.0, "wz": 10.2244787521301},
    {"step": 13, "brick": "B", "side": "R", "wx": 47.0, "wz": 1.5642247142857144},
    {"step": 14, "brick": "E'", "side": "R", "wx": 37.5, "wz": 1.5642247142857144},
    {"step": 15, "brick": "C", "side": "R", "wx": 41.0, "wz": 2.8014038625491984},
    {"step": 16, "brick": "E'", "side": "R", "wx": 46.5, "wz": 2.801403862549198},
    {"step": 17, "brick": "C'", "side": "R", "wx": 36.5, "wz": 3.4199934366809397},
    {"step": 18, "brick": "F", "side": "R", "wx": 39.5, "wz": 4.657172584944423},
    {"step": 19, "brick": "A", "side": "R", "wx": 49.5, "wz": 4.657172584944423},
    {"step": 20, "brick": "D", "side": "R", "wx": 47.0, "wz": 6.512941307339648},
    {"step": 21, "brick": "B'", "side": "R", "wx": 35.0, "wz": 5.894351733207907},
    {"step": 22, "brick": "D", "side": "R", "wx": 39.5, "wz": 7.131530881471391},
    {"step": 23, "brick": "B'", "side": "R", "wx": 32.0, "wz": 7.131530881471392},
    {"step": 24, "brick": "F", "side": "R", "wx": 36.5, "wz": 8.368710029734876},
    {"step": 25, "brick": "C'", "side": "R", "wx": 30.5, "wz": 8.368710029734876},
    {"step": 26, "brick": "B", "side": "L", "wx": 26.0, "wz": 10.2244787521301},
]
...
def source_pose(source_side, grid_id):
    return (LEFT_GRID if source_side == "L" else RIGHT_GRID)[grid_id]
...
def source_for_step(move, use_counts):
    ...
    return source_side, grid_id, used >= len(pool)


def load_state():
    steps = []
    use_counts = {}
    for move in CONSERVATIVE_002:
    ...
    state = {
        "version": 1,
        "updated_at": now_iso(),
        "notes": "Measured real-world placement poses for conservative_002.",
        "steps": steps,
    }
...
def print_step(step):
    ...
    print("source {} {} pose={}{}".format(step["source_side"], step["source_grid_id"], step["source_pose"], refill))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Tune conservative_002 real-world placement poses.")
    parser.add_argument("--start-step", type=int, default=None, help="start from this conservative_002 step number")
        ...
        while not rospy.is_shutdown():
            ...
            if c == "q":
                ...
                if index >= len(state["steps"]) - 1:
                    print("All {} steps measured. saved {}".format(len(state["steps"]), OUT_PATH))
                    return
                index += 1
                break
