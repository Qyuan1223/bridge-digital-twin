...
try:
    from ranker_features import extract_features
    from train_ranker import dot as ranker_dot
except Exception:
...
ML_MODEL_DIR = Path(__file__).with_name("models")
ML_MODEL_PATHS = {
    "aggressive": ML_MODEL_DIR / "aggressive_ranker.json",
    "conservative": ML_MODEL_DIR / "conservative_ranker.json",
}
ML_RANKER_MODELS: Dict[str, Dict[str, Any]] = {}
ML_RANKER_FAILED: set[str] = set()
...
def geometric_options_for_hover(
    ...
    return sorted(
        ...
        key=lambda o: (o.wz, -contact_width(o.contacts)),
    )
...
def ml_load_ranker(strategy: str) -> Optional[Dict[str, Any]]:
    if strategy in ML_RANKER_MODELS:
        return ML_RANKER_MODELS[strategy]
    if strategy in ML_RANKER_FAILED:
        return None
    if extract_features is None or ranker_dot is None:
        ML_RANKER_FAILED.add(strategy)
        log_event("ML", "ranker modules unavailable; using heuristic recommendations")
        return None
    path = ML_MODEL_PATHS.get(strategy)
    if path is None:
        ML_RANKER_FAILED.add(strategy)
        log_event("ML", f"no model path for strategy={strategy}; using heuristic recommendations")
        return None
    try:
        model = json.loads(path.read_text(encoding="utf-8"))
        ML_RANKER_MODELS[strategy] = model
        log_event("ML", f"loaded {path.name} blend={model.get('rule_blend_weight', 0.0)}")
        return model
    except OSError as exc:
        ML_RANKER_FAILED.add(strategy)
        log_event("ML", f"model not loaded: {exc}; using heuristic recommendations")
        return None
    except json.JSONDecodeError as exc:
        ML_RANKER_FAILED.add(strategy)
        log_event("ML", f"model JSON invalid: {exc}; using heuristic recommendations")
        return None


def ml_previous_reason(bridge: "Bridge") -> str:
    if not bridge.bricks:
        return ""
    before = Bridge()
    for b in bridge.bricks[:-1]:
        before.bricks.append(BrickInst(b.id, b.btype, b.wx, b.wz, b.actor, list(b.contacts), b.margin))
        before.next_id = max(before.next_id, b.id + 1)
    before.analyse()
    after = bridge
    last = bridge.bricks[-1]
    side = side_for_x((last.x0 + last.x1) / 2.0)
    if after.bridge_closed:
        return "close_bridge"
    if side_reach_gain(before, after, side) > 0.05:
        return "effective_cantilever"
    if is_foundation_move(last):
        return "foundation_fill"
    if before.joints and after.min_margin > before.min_margin + 0.10:
        return "repair_weakness"
    return "rear_counterweight"


def ml_bridge_state_features(bridge: "Bridge") -> Dict[str, Any]:
    max_height = max((b.wz + b.btype.height for b in bridge.bricks), default=0.0)
    return {
        ...
        "reach_balance": reach_balance(bridge),
        "left_foundation_coverage": foundation_coverage_for_side(bridge, "L"),
        "right_foundation_coverage": foundation_coverage_for_side(bridge, "R"),
        "left_base_utilization": base_cell_utilization_for_side(bridge, "L"),
        "right_base_utilization": base_cell_utilization_for_side(bridge, "R"),
        "left_void_score": base_trapped_void_score_for_side(bridge, "L"),
        "right_void_score": base_trapped_void_score_for_side(bridge, "R"),
        "max_height": max_height,
        "no_progress_streak": no_progress_streak(bridge),
...
def ml_contact_payload(contacts: Sequence[SurfaceSeg]) -> List[Dict[str, Any]]:
    return [
        {
            "x0": c.x0,
            "x1": c.x1,
...
def ml_brick_payload(bridge: "Bridge") -> List[Dict[str, Any]]:
    return [
        {
            "id": b.id,
            "brick": b.btype.key,
            ...
            "contacts": ml_contact_payload(b.contacts),
        }
        for b in bridge.bricks
    ]


def ml_candidate_contact_features(before: "Bridge", option: BrickInst) -> Dict[str, Any]:
    owners = {c.owner for c in option.contacts}
    left_component = before.component_from(BASE_LEFT)
    right_component = before.component_from(BASE_RIGHT)
...
def ml_candidate_features(before: "Bridge", rec: Rec) -> Dict[str, Any]:
    side = side_for_x(rec.wx + brick_bounds_center(rec.bt))
    option = BrickInst(before.next_id, rec.bt, rec.wx, rec.wz, "robot", list(rec.contacts))
    sim = before.clone()
    bi = BrickInst(sim.next_id, rec.bt, rec.wx, rec.wz, "robot", list(rec.contacts))
    sim.bricks.append(bi)
    sim.next_id += 1
    sim.analyse()

    closes_bridge = sim.is_closing_brick(bi.id)
    stable_by_model = closes_bridge or sim.min_margin >= MIN_ACCEPT_MARGIN
    base_margin = before.min_margin if before.joints else 1.0
    side_util_before = base_cell_utilization_for_side(before, side)
    side_util_after = base_cell_utilization_for_side(sim, side)
    side_void_before = base_trapped_void_score_for_side(before, side)
    side_void_after = base_trapped_void_score_for_side(sim, side)
    foundation_before = foundation_average_coverage(before)
    foundation_after = foundation_average_coverage(sim)
    ...
    return {
        ...
        "contact_width": contact_width(rec.contacts),
        **ml_candidate_contact_features(before, option),
        "support_efficiency": foundation_support_efficiency(option),
        "is_foundation_move": is_foundation_move(option),
        "low_height_bonus": low_height_bonus(option),
        "height_penalty": premature_height_penalty(before, option),
        "margin_after": finite_margin(sim.min_margin),
        "margin_delta": finite_margin(sim.min_margin) - finite_margin(base_margin),
        "reach_gain": side_reach_gain(before, sim, side),
        "closure_gain": max(0.0, before.remaining_gap - sim.remaining_gap),
        "gap_coverage_after": sim.gap_coverage,
        "balance_gain": reach_balance(before) - reach_balance(sim),
        ...
        "contacts": ml_contact_payload(rec.contacts),
    }


def ml_score_rec(bridge: "Bridge", rec: Rec, model: Dict[str, Any], previous_reason: str, strategy: str) -> float:
    case = {
        "before_state": ml_bridge_state_features(bridge),
        "before_bricks": ml_brick_payload(bridge),
    }
    candidate = ml_candidate_features(bridge, rec)
    features = extract_features(case, candidate, {"previous_reason": previous_reason})
    learned = ranker_dot(model.get("weights", {}), features)
    blend = float(model.get("rule_blend_weight", 0.0))
    rule_key = "rule_conservative_score" if strategy == "conservative" else "rule_aggressive_score"
    return learned + blend * features.get(rule_key, 0.0)


def ml_rerank_recs(bridge: "Bridge", strategy: str, recs: Sequence[Rec]) -> List[Rec]:
    if strategy not in ("aggressive", "conservative") or not recs:
        return list(recs)
    model = ml_load_ranker(strategy)
    if model is None:
        return list(recs)
    previous_reason = ml_previous_reason(bridge)
    ranked: List[Rec] = []
    for rec in recs:
        cp = copy_rec(rec)
        try:
            cp.score = ml_score_rec(bridge, cp, model, previous_reason, strategy)
            if "/ml" not in cp.intent:
                cp.intent = f"{cp.intent}/ml"
        except Exception as exc:
            log_event("ML", f"score failed for {cp.bt.key} wx={cp.wx:.2f}: {exc}")
            cp.score = rec.score
        ranked.append(cp)
    ranked.sort(key=lambda r: (-r.score, -r.margin_after, -r.reach_gain, r.wz, r.side, r.bt.key, r.wx))
    return ranked


def bottom_first_recs(bridge: "Bridge", recs: Sequence[Rec]) -> List[Rec]:
    """Keep same-side recommendations from skipping usable lower placements.

    The recorder lets the user place any geometric drop. For robot suggestions
    we still need a behavioral rule: while a side has stable low-layer options,
    do not recommend non-closing high-layer moves on that same side.
    """
    if not recs:
        return []
    grouped: Dict[str, List[Rec]] = {"L": [], "R": []}
    for rec in recs:
        grouped.setdefault(rec.side, []).append(rec)

    kept: List[Rec] = []
    for side, side_recs in grouped.items():
        if not side_recs:
            continue
        closing = [rec for rec in side_recs if rec.intent == "close"]
        normal = [rec for rec in side_recs if rec.intent != "close"]
        if not normal:
            kept.extend(closing)
            continue
        lowest_wz = min(rec.wz for rec in normal)
        same_low_band = lowest_wz + HEX_RISE * 1.05
        lowest_layer = [rec for rec in normal if rec.wz <= same_low_band]
        kept.extend(closing + lowest_layer)
    return kept
...
def rec_is_placeable(bridge: Bridge, rec: Rec) -> bool:
    ...
    if not support_contact_ok(bi, closes_bridge):
        return False
    if not (stable_enough_margin(sim.min_margin) or closes_bridge):
        return False
    return True
...
def recommend(bridge: Bridge, strategy: str, top_n: int = 4) -> List[Rec]:
    cache_key = ("recommend_ml_test", bridge_state_key(bridge), strategy, top_n)
    cached = RECOMMEND_CACHE.get(cache_key)
    if cached is not None:
        return [copy_rec(rec) for rec in cached]

    cands = recommend_immediate(bridge, strategy, limit=max(24, top_n * 8))
    lookahead_weight = 0.35 if strategy == "conservative" else 0.55
    for rec in cands[: max(top_n, min(12, len(cands)))]:
        sim = bridge.clone()
        if sim.place(rec.bt, rec.wx, "robot", rec.wz):
            rec.score += lookahead_weight * future_potential(sim, strategy)
    if strategy != "aggressive":
        cands = bottom_first_recs(bridge, cands)
        cands = ml_rerank_recs(bridge, strategy, cands)
...
class UI:
    ...
    def render(
        ...
        hover_option: Optional[BrickInst],
        hover_count: int,
        drop_index: int,
    ) -> None:
        self.sc.fill(BG)
        self.draw_canvas(bridge, recs, sel, hover_wx, hover_option, hover_count, drop_index, message, reaction)
    ...
    def draw_canvas(
        ...
        if hover_option is not None:
            self.draw_ghost(hover_option.btype, hover_option.wx, hover_option.wz, 22, outline_alpha=125)
            if hover_count > 1:
                label = self.f12.render(
                    f"drop {drop_index + 1}/{hover_count}",
                    True,
                    PANEL_MUTED,
                )
                px, py = w2p(hover_option.world_cx, hover_option.wz + hover_option.btype.height + 0.45)
    ...
    def draw_panel(self, bridge: Bridge, recs: List[Rec], sel: str, strategy: str, message: str, reaction: str) -> None:
        ...
        row("Bridge Twin ML Test", self.f28, PANEL_TEXT, 4)
...
def main() -> None:
    ...
    pygame.display.set_caption("Bridge Digital Twin - ML Test")
    ...
    while True:
        ...
        hover_wx_raw = p2w(mx, my)[0] if mx < PANEL_X else None
        hover_wx: Optional[float] = None
        hover_options: List[BrickInst] = []
        hover_option: Optional[BrickInst] = None
        if hover_wx_raw is not None and selected in BTYPES:
            bt = BTYPES[selected]
            hover_wx = snap_place_x(hover_wx_raw - brick_bounds_center(bt))
        ...
        for ev in pygame.event.get():
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1 and mx < PANEL_X:
                ...
                if hover_option is None:
                    ...
                    reaction = "shake"
                    wx = hover_wx if hover_wx is not None else p2w(mx, my)[0]
        ...
        ui.render(bridge, recs, selected, strategy, message, reaction, hover_wx, hover_option, len(hover_options), drop_index)
        clock.tick(30)
