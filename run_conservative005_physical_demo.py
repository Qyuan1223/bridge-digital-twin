#!/usr/bin/env python3
"""
Run a fully robotic physical test of conservative_005.
...
Run on JetMax:
    rosrun jetmax_demos run_conservative005_physical_demo.py --step-confirm
"""
...
ROS_NODE_NAME = "run_conservative005_physical_demo"
...
SAFE_Z = 180.0
APPROACH_LIFT = 35.0
PICK_TRANSIT_Z = 150.0
PICK_LIFT = 45.0
PLACE_TRANSIT_Z = 200.0
MAX_TRANSIT_Z = 205.0
BYPASS_Y = -95.0
ENTRY_BACKOFF_Y = 25.0
EXIT_LIFT = 18.0
TRANSIT_GRIPPER = 75.0
...
CONSERVATIVE_005 = [
    {"step": 1, "brick": "B'", "side": "L", "wx": 6.5, "wz": 1.5642247142857144},
    {"step": 2, "brick": "B'", "side": "L", "wx": 15.5, "wz": 1.5642247142857142},
    {"step": 3, "brick": "B'", "side": "R", "wx": 39.5, "wz": 1.5642247142857144},
    {"step": 4, "brick": "D", "side": "R", "wx": 47.0, "wz": 1.5642247142857144},
    {"step": 5, "brick": "A", "side": "R", "wx": 48.0, "wz": 2.8014038625491984},
    {"step": 6, "brick": "D", "side": "L", "wx": 8.0, "wz": 2.8014038625491984},
    {"step": 7, "brick": "C'", "side": "L", "wx": 17.0, "wz": 2.801403862549198},
    {"step": 8, "brick": "E", "side": "R", "wx": 39.0, "wz": 2.8014038625491984},
    {"step": 9, "brick": "D", "side": "R", "wx": 36.5, "wz": 4.657172584944424},
    {"step": 10, "brick": "E", "side": "R", "wx": 48.0, "wz": 4.0385830108126815},
    {"step": 11, "brick": "F", "side": "R", "wx": 47.0, "wz": 5.275762159076166},
    {"step": 12, "brick": "C'", "side": "R", "wx": 42.5, "wz": 5.894351733207907},
    {"step": 13, "brick": "B'", "side": "R", "wx": 35.0, "wz": 5.894351733207907},
    {"step": 14, "brick": "C", "side": "R", "wx": 45.5, "wz": 7.13153088147139},
    {"step": 15, "brick": "D", "side": "R", "wx": 33.5, "wz": 7.131530881471392},
    {"step": 16, "brick": "E", "side": "R", "wx": 37.5, "wz": 8.368710029734876},
    {"step": 17, "brick": "A", "side": "R", "wx": 45.0, "wz": 8.987299603866617},
    {"step": 18, "brick": "A", "side": "R", "wx": 34.5, "wz": 9.605889177998359},
    {"step": 19, "brick": "D", "side": "R", "wx": 41.0, "wz": 10.224478752130102},
    {"step": 20, "brick": "C", "side": "R", "wx": 38.0, "wz": 11.461657900393584},
    {"step": 21, "brick": "F", "side": "L", "wx": 11.0, "wz": 4.0385830108126815},
    {"step": 22, "brick": "C'", "side": "L", "wx": 18.5, "wz": 4.657172584944423},
    {"step": 23, "brick": "D", "side": "L", "wx": 9.5, "wz": 5.894351733207907},
    {"step": 24, "brick": "E'", "side": "L", "wx": 18.0, "wz": 5.894351733207907},
    {"step": 25, "brick": "C", "side": "L", "wx": 15.5, "wz": 7.131530881471391},
    {"step": 26, "brick": "D", "side": "L", "wx": 6.5, "wz": 7.131530881471391},
    {"step": 27, "brick": "B", "side": "L", "wx": 9.5, "wz": 8.368710029734876},
    {"step": 28, "brick": "A", "side": "L", "wx": 7.5, "wz": 9.60588917799836},
    {"step": 29, "brick": "F'", "side": "L", "wx": 20.0, "wz": 7.750120455603133},
    {"step": 30, "brick": "C'", "side": "L", "wx": 14.0, "wz": 8.987299603866619},
    {"step": 31, "brick": "E'", "side": "L", "wx": 22.5, "wz": 8.987299603866617},
    {"step": 32, "brick": "F", "side": "L", "wx": 17.0, "wz": 10.224478752130103},
    {"step": 33, "brick": "B", "side": "L", "wx": 27.5, "wz": 10.843068326261847},
]
...
def pick_clearance_z(z, args):
    return min(args.pick_transit_z, z + args.pick_lift)


def place_clearance_z(z, args):
    clear_z = min(args.max_transit_z, args.place_transit_z)
    if clear_z <= z:
        clear_z = min(args.max_transit_z, z + 5.0)
    return clear_z
...
def reset_home(args, holding):
    ...
    if holding:
        cur = jetmax.position
        lift_z = max(cur[2], min(args.pick_transit_z, cur[2] + args.pick_lift))
        print("  holding return via ({:.1f}, {:.1f}, {:.1f}) before yaw reset".format(cur[0], cur[1], lift_z))
        move_xyz(cur[0], cur[1], lift_z, 0.55)
        move_xyz(args.home_x, args.home_y, lift_z, travel_time(cur, (args.home_x, args.home_y, lift_z)))
        move_xyz(args.home_x, args.home_y, args.home_z, 0.75)
...
def move_to_work_pose(x, y, z, yaw, args, label):
    local_z = pick_clearance_z(z, args)
    print("{} ({:.1f}, {:.1f}, {:.1f}) yaw={:.1f} pick_clearance_z={:.1f}".format(label, x, y, z, yaw, local_z))
    set_servo1(yaw, 0.3)
    cur = jetmax.position
    if cur[2] < local_z:
        move_xyz(cur[0], cur[1], local_z, 0.45)
    move_xyz(x, y, local_z, travel_time(cur, (x, y, local_z)))
    move_xyz(x, y, z, 0.6)
    return local_z


def move_to_place_pose(x, y, z, yaw, args, label):
    clear_z = place_clearance_z(z, args)
    bypass_y = args.bypass_y
    entry_y = y + args.entry_backoff_y
    print("{} ({:.1f}, {:.1f}, {:.1f}) yaw={:.1f} clearance_z={:.1f} bypass_y={:.1f} entry_y={:.1f}".format(
        label, x, y, z, yaw, clear_z, bypass_y, entry_y
    ))
    set_servo1(yaw, 0.3)
    cur = jetmax.position
    move_xyz(x, bypass_y, clear_z, travel_time(cur, (x, bypass_y, clear_z)))
    move_xyz(x, entry_y, clear_z, max(abs(entry_y - bypass_y) / 120.0, 0.5))
    move_xyz(x, y, z, max(args.place_descent_time, 0.75))
    return clear_z


def leave_place_pose(x, y, z, clear_z, args):
    bypass_y = args.bypass_y
    entry_y = y + args.entry_backoff_y
    lift_z = min(clear_z, z + args.exit_lift)
    move_xyz(x, y, lift_z, 0.45)
    move_xyz(x, y, clear_z, 0.55)
    set_gripper(args.transit_gripper, 0.25)
    move_xyz(x, entry_y, clear_z, max(abs(entry_y - y) / 120.0, 0.35))
    move_xyz(x, bypass_y, clear_z, max(abs(entry_y - bypass_y) / 120.0, 0.5))
...
def place_brick(item, args):
    ...
    clear_z = move_to_place_pose(x, y, z, yaw, args, "PLACE step {:02d} {} [{}]".format(
        item["step"], item["brick"], item["place_source"]
    ))
    set_gripper(args.release, 0.6)
    rospy.sleep(args.release_hold)
    leave_place_pose(x, y, z, clear_z, args)


def print_plan(plan):
    print("Conservative 005 physical demo plan:")
    for item in plan:
...
def parse_args():
    p = argparse.ArgumentParser(description="Run full-machine conservative_005 physical smoke test.")
    ...
    p.add_argument("--end-step", type=int, default=33)
    ...
    p.add_argument("--pick-transit-z", type=float, default=PICK_TRANSIT_Z, help="reachable source-pick transit height")
    p.add_argument("--pick-lift", type=float, default=PICK_LIFT, help="lift above source brick before returning home")
    p.add_argument("--place-transit-z", type=float, default=PLACE_TRANSIT_Z)
    p.add_argument("--max-transit-z", type=float, default=MAX_TRANSIT_Z)
    p.add_argument("--bypass-y", type=float, default=BYPASS_Y, help="rear Y corridor used to avoid the bridge plane")
    p.add_argument("--entry-backoff-y", type=float, default=ENTRY_BACKOFF_Y, help="stop this far behind the target before the final diagonal descent")
    p.add_argument("--exit-lift", type=float, default=EXIT_LIFT, help="empty-gripper vertical lift after release")
    p.add_argument("--transit-gripper", type=float, default=TRANSIT_GRIPPER, help="compact gripper angle for exiting after release")
    p.add_argument("--place-descent-time", type=float, default=1.0, help="duration for the final diagonal/near-vertical descent")
